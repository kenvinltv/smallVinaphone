<?php
	...
	{
	   //----------------- Page -----------------------------//
		public function ListPAActive() {
			$sql="	SELECT *
					FROM pages
					WHERE AnHien = 1 AND NoiDung != ''
				";
        ...
        public function ThemPA()
		{
			...
			$sql="INSERT INTO pages (TieuDe, TieuDeKD, NoiDung, Des, idGroup, NgayDang, Title, Keyword,UrlHinh,TomTat,linkchuyen,co)
				VALUES ('$TieuDe', '$TieuDeKD', '$NoiDung', '$Des', $idGroup , '$NgayDang', '$Title', '$Keyword', '$UrlHinh', '$TomTat','$linkchuyen','$co')";
		...
		public function SuaSetting()
		{
            $Phone = $_POST['Phone'];
			$UrlFace = $_POST['UrlFace'];
			$UrlGlus = $_POST['UrlGlus'];
			$UrlTwis = $_POST['UrlTwis'];
		...
		public function ThemTT(){
			...
			//chèn vào db
			$sql="INSERT INTO   tintuc (idLoai, idCL, idCon, TieuDe, TieuDeKD, TomTat, NoiDung, Title, Keyword, Des, UrlHinh, NgayDang)
			  		VALUES ($idLoai, $idCL, $idCon, '$TieuDe', '$TieuDeKD', '$TomTat', '$NoiDung', '$Title', '$Keyword', '$Des', '$UrlHinh', '$NgayDang')";
			mysql_query($sql) or die(mysql_error());
            ...
            if ($UrlHinh!= '')
			{
				$file = basename($UrlHinh);
				parent::resizeform($file, 84, 50);
				parent::resizeform($file, 103, 62);
				parent::resizeform($file, 159, 121);
				parent::resizeform($file, 215, 108);
			}
		}
        ...
        public function ListTK(&$totalRows, $pageNum, $pageSize, $TieuDe){
			$startRow = ($pageNum-1)*$pageSize;
			if ($TieuDe != ""){
				$sql="	SELECT idUser, User, Name, BoPhan, idGroup
				FROM  users
        ...
        public function ThemTK()
		{
			...
			$Name = parent::XoaDinhDang($Name);
			$BoPhan = parent::XoaDinhDang($BoPhan);
			$Pass = parent::XoaDinhDang($Pass);
			$Pass = md5($Pass);
			settype($idGroup,"int");

			//Chèn dữ liệu vào database
			$sql="INSERT INTO users (User, Name, BoPhan, Pass, idGroup)
				VALUES ('$User', '$Name', '$BoPhan', '$Pass', '$idGroup')";
			mysql_query($sql) or die (mysql_error());
		}
		...
		public function SuaTKInfo($id, $name, $bophan){
			$name = parent::XoaDinhDang($name);
			$bophan = parent::XoaDinhDang($bophan);
			$sql="UPDATE users
				SET Name = '$name', BoPhan = '$bophan'
                WHERE idUser = $id
                ";
			mysql_query($sql) or die(mysql_error());
		}
