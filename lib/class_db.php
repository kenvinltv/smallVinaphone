...
class db {
	...
	public function ListQuangCao($idLoai){        
		$sql="SELECT TieuDe, UrlHinh, Link
            from quangcao
            WHERE  AnHien = 1 and idLoai = $idLoai
			ORDER BY ThuTu asc
			";
		$kq = mysql_query($sql,$this->conn);
		$sql="SELECT count(*) FROM quangcao WHERE AnHien = 1 and idLoai = $idLoai
			";
		$rs= mysql_query($sql);
	...
	function pagesList1($baseURL, $totalRows , $pageNum=1, $pageSize = 5, $offset = 5){
		...
		if ($pageNum > 1) {
			$firstLink = "<li class='first'><a href='$baseURL/' title='Trang đầu'><<</a></li>";
			$prevPage = $pageNum - 1;
			$prevLink="<li><a href='$baseURL/page-$prevPage.html' title='Lùi'>«</a></li>";
		}
		...
		for($j = $from; $j <= $to; $j++) {
			{
				if ($j==$pageNum)
					$links= $links . "<li class='current'>$j</li>";
				else{
					if ($j == 1)
						$links= $links . "<li><a href = '$baseURL/' title='Trang $j'>$j</a></li>";
					else
						$links= $links . "<li><a href = '$baseURL/page-$j.html' title='Trang $j<'>$j</a></li>";
				}
					
			}
		} //for
		if ($pageNum < $totalPages) {
			$lastLink = "<li class='last'><a class='phantrang_hinh' href='$baseURL/page-$totalPages.html' title='Trang cuối'>>></a></li>";
			$nextPage = $pageNum + 1;
			$nextLink = "<li><a class='phantrang_hinh' href='$baseURL/page-$nextPage.html' title='Tới'>»</a></li>";
		}
		return '<ul class="pagination">'.$firstLink.$prevLink.$links.$nextLink.$lastLink.'</ul>';
	} // function pagesList1
    ...
    function resizeform($file,$NewImageWidth,$NewImageHeight)
    {
        ini_set('max_execution_time', 0);
        $ImagesDirectory    = $_SERVER["DOCUMENT_ROOT"].'/upload/hinhanh/';
        $DestImagesDirectory    = $_SERVER["DOCUMENT_ROOT"].'/upload/resize/'.$NewImageWidth.'x'.$NewImageHeight.'/'; 
